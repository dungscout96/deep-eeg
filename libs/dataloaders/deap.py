...
class DeapTransform(ABC):
    data_key_map = {
        "feat_inst_theta": (4., 8.),
        "feat_inst_alpha": (8., 13.),
        "feat_inst_beta": (13., 30.),
        # "feat_inst_theta": (3., 7.),
        # "feat_inst_alpha": (8., 13.),
        # "feat_inst_beta": (14., 29.),
        # "feat_inst_gamma": (30., 47.),
    }
...
class AvgFreqPower(DeapTransform):
    ...
    def transform(self, data):
        ...
        for k in self.data_keys:
            filt_data = [mne.filter.filter_data(d.T, sfreq=256., 
                            l_freq=DeapTransform.data_key_map[k][0], 
                            h_freq=DeapTransform.data_key_map[k][1], 
...
class TopomapNN(DeapTransform, NNUtils):
...
class TopomapImg(DeapTransform):
    data_keys = ["feat_inst_theta", "feat_inst_alpha", "feat_inst_beta"] # R G B
...
class Spectrogram(DeapTransform):
    data_keys = ["data"]
...
class SpectrogramImg(DeapTransform):
    data_keys = ["data"]
...
class DeapDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        self.sessions = np.array([i.split('.')[0] for i in os.listdir(self.basedir) if i.split('.')[-1] == 'dat'])
        if sessions != None:
            self.sessions = np.array([i for i in sessions if i in self.sessions])
        ...
        standard_order = mne.channels.make_standard_montage('biosemi32').ch_names
        remap_ch_idx = [original_order.index(ch) for ch in standard_order]
        ...
        for y_key in y_keys:
            if y_key not in DeapDataset.BEHAVIOR_KEYS:
        ...
        # Preload data
        raw_data = [self.__preload_raw(session, remap_ch_idx) for session in tqdm(self.sessions)]
        raw_x = [x for x,_ in raw_data] # Session S T F
        raw_y = [y for _,y in raw_data] # Session S len(y_keys)
    ...
    def __preload_raw(self, session, remap_ch_idx=None):
        with open(os.path.join(self.basedir, session) + '.dat', 'rb') as fin:
            data = pickle.load(fin, encoding='latin1')
        y_take = [DeapDataset.BEHAVIOR_KEYS.index(k) for k in self.y_keys]
        if remap_ch_idx is None:
            remap_ch_idx = list(range(data["data"].shape[1]))
        # video/trial x channel x data; video/trial x label (valence, arousal, dominance, liking)
        x = data["data"][:,remap_ch_idx,:] # S F T
        x = np.swapaxes(x, 1, 2) # S F T -> S T F
        y = data["labels"][:,y_take] # S len(y_keys)
        return x, y
    ...
    def __transform_y(self, data):
        self.y_data = np.array(data, dtype=np.int8)
        shape = self.y_data.shape
        self.y_data = np.reshape(self.y_data, (shape[0]*shape[1], *shape[2:]))
